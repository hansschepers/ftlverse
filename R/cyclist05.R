...
cyclist05 <- function(time = 0
  ...
  # pa <- .pa
  # pa["T_air"]
  stopifnot(all(c("T_air", "hAH", "Slope") %in% names(pa)))
  # print(pa["treadmill"])  
  # CONSTANTS <- get(CONSTANTS, .GlobalEnv)
  ...
  # more aux not needing states...
  {
    ...
    # if (phase == 1) 
    {
      ...
      WORKEQ <- max(pa["BASAL_METABOLIC_RATE"]
                    , partVO2 * pa["L2kcal"] * 60)
  ...
  pa["v_wind"] <- pa["v_wind"] * pa["treadmill"]
  ...
  for (n in 1:25) {
    ...
    # if (n == 1) 
    # print(paste(n, error[n], cold[n], warm[n], y[n], TSET[n]))
  ...
  ####################################################### VELOCITY CHANGE (ACCELLERATION) #######################
  {
    ...
    # if (phase == 1) 
    {
      ...
      # print(paste(time, v_cycle, unname(Pacc), unname(Pext), unname(Pair), unname(Prolling), unname(Pgrade)))
      # Pacc can be constrained to provide forward thrust
      if(Pacc < 0) {
        Pacc <- 0
        if (!exists("ii_negative_time")) ii_negative_time <<- 1
  ...
  # print(hprettyNum(as.numeric(ydot)[c(1, 2, 3, 4, 25, 26, 27)], digits = 2))
  # print(as.numeric(ydot)[c(1, 2, 3, 4, 25, 26, 27)], digits = 2)
  # print(y[c(1, 2, 3, 4, 25, 26, 27)], digits = 2)
