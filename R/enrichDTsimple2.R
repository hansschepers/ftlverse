...
enrichDTsimple2 <- function(DT
  ...
  if (is.null(variable.factor)) variable.factor <- is.factor(DT[, ..variable])
  ...
  if (!is.null(expression)) {
    if (inherits(expression, "list")){
      ...
      for (iexpr in expression){
        ...
        wideDT[, rlang::eval_tidy(iexpr), by = setdiff(bycols, variable)]
      }
    } else {
      wideDT[, rlang::eval_tidy(expression), by = setdiff(bycols, variable)]  # , parent.frame = 
    }
  }
