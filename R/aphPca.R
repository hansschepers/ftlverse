...
#' @export
aphPca <- function (dfg
                    , subtractTimevarFromYois = FALSE
                    , yois = aphVariableLevels(dfg, subtractTimes = subtractTimevarFromYois)
                    , foi = aphFactors(dfg)
                    , labelCol = NULL
                    , labelRepel = 0
                    , color = NULL
                    ...
                    , ...) 
{
  ...
  if (length(labelCol)) {
    if (!labelCol %in% names(m)) {
      ...
      dfg[, labelCol] <- labelCol
      # labelCol <- foi
    }
  }
  if (length(labelCol)) {
    if (labelCol == "none") labelCol <- NULL
    if (labelCol == foi) labelCol <- NULL
  }
  
  numericCols <- aphVariableLevels(dfg, direction = "wide", subtractTimes = subtractTimevarFromYois)
  ...
  if (doScale == 1) {
    if ("scale" %in% names(kb.local[[data.oi]][["outpca"]])) {
      log_info("using scale from kb.local")
      scale <- kb.local[[data.oi]][["outpca"]][["scale"]]
    }
    if ("center" %in% names(kb.local[[data.oi]][["outpca"]])) {
      log_info("using center from kb.local")
      center <- kb.local[[data.oi]][["outpca"]][["center"]]
    }
    ms <- scale(m, center = center, scale = scale)
  } else {
    ms <- m
  }
