...
#' }
#' @export
recbox01PostProcess <- function(out = data.frame()
                        , parms = recbox01Parms()
                        , timeSeries = FALSE){
  # parmsUsed <- .parmsUsed
  ...
  #     out <- data.table(time = seq(0, 5*TT, length.out = 1001))
  ...
  # out[, u := (k_1/L1 + FF*k_1    * gt)      / (1 + gt)]
  # out[, v := (   k_1 + FF*k_1*L2 * gt * cc) / (1 + gt * cc)]
  
  ################################################################## instantaneous
  # out[, a := (a1 + a2 * gt)      / (1 + gt)]
  # out[, b := (a4 + a3 * gt * cc) / (1 + gt * cc)]
  # out[, Act := a * r + b * (1 - r)]
  
  
  ################################################################## continuous
  out[, uc := (k1  + k2  * gc)      / (1 + gc)]
  out[, vc := (k_1 + k_2 * gc * cc) / (1 + gc * cc)]
  out[, rss := vc / (uc + vc)]
  out[, ac := (a1 + a2 * gc)      / (1 + gc)]
  out[, bc := (a4 + a3 * gc * cc) / (1 + gc * cc)]
  out[, M_c := ac * rss + bc * (1 - rss)]
